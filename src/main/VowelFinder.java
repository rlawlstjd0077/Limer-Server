...
public class VowelFinder {
    ...
    public static String lastCharJasoAtom(char ch) {
        StringBuilder rv = new StringBuilder(ch * 6);

        if (ch >= '가' && ch <= '힣') {
            // 한글의 시작부분을 구함
            int ce = ch - '가';
            // 초성을 구함
            rv.append(KO_ATOM_S[ce / (588)]);
    ...
    public static String toKoMoumAtom(String text) {
        if (text == null) {
            return null;
        }
